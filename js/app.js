import * as THREE from 'three/webgpu';
		...
		// Force dedicated GPU — always request high-performance when not low-end.
		// 'powerPreference: high-performance' is the API that tells the browser
		// to use the dedicated GPU (NVIDIA/AMD) instead of integrated (Intel/UHD).
		// Browser decides which adapter to use — if dedicated exists, it picks that.
		...
		// Warm up a WebGL context with high-performance to force browser GPU init
		(function warmGPU() {
			try {
		...
		// Promote visible videos to GPU compositor layer for smooth playback
		...
		document.querySelectorAll('video').forEach(v => _videoObserver.observe(v));
		...
		const renderer = new THREE.WebGPURenderer({ antialias: !isLowEnd, powerPreference: powerPref });
		...
		const stageNames = ['Hero', 'Manifesto', 'Portfolio', 'Stats', 'Quote', 'CTA', 'Footer'];
		...
		// Scroll to #about hash on load (from other pages)
		if (window.location.hash === '#about') {
			const aboutEl = document.getElementById('about');
			if (aboutEl) setTimeout(() => aboutEl.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
		}

		// Handle hash changes (back/forward, same-page nav)
		...
		function animate(timestamp) {
			...
			if (_siteFooter) {
				...
				if (footerTop < window.innerHeight) {
					...
					if (renderer.domElement._lastTy !== ty) {
						renderer.domElement.style.transform = `translateY(${ty}px)`;
						renderer.domElement._lastTy = ty;
					}
				} else if (renderer.domElement._lastTy !== 0) {
					renderer.domElement.style.transform = '';
					renderer.domElement._lastTy = 0;
				}
			}
